#install.packages(c('BioGeoBEARS','optimx','FD','snow','parallel','ape'))
...
setwd('~/Desktop/Cyrt_BGB/Reduced')

#see where example data file are on your computer
np(system.file("extdata", package="BioGeoBEARS"))
###Make sure to prune outgroups from tree like this
#but we are not using this tree today bc we are using the ex. files so ignore this!:
#prune phylogeny to remove outgroups
###prune reduced
tree2 <- read.tree("Expanded.tree")
#feed a list of tip you want to keep!
species = c("18642_C_Daun","18772_C_lateralis","C_adorus","C_Agam_19635","C_agusanensi","C_angularis","C_annandalei","C_annulatus","C_arcanus","C_astrum","C_aurensis","C_australotitiwangsaensis","C_ayeyarwadyensis","C_badenensis","C_baluensis","C_bansocensis","C_batucolus","C_bichnganae","C_bidoupimontis","C_bintangrendah","C_bintangtinggi","C_bobrovi","C_boreoclivus","C_brevidactylus","C_bugiamapensis","C_calamei","C_caovansungi","C_capreoloides","C_cattienensis","C_cavernicolus","C_chanhomeae","C_chrysopylos","C_condorensis","C_consobrinus","C_cryptus","C_cucdongensis","C_cucphuongensis","C_darmandvillei","C_dati","C_durio","C_eisenmanae","C_elok","C_epiroticus","C_fasciolatum","C_feae","C_gansi","C_grismeri","C_guakanthanensis","C_gubernatoris","C_gunungsenyumensis","C_hidupselamanya","C_hinnamnoensis","C_hontreensis","C_hoskini","C_huongsonensis","C_huynhi","C_interdigitalis","C_intermedius","C_irregularis","C_jaegeri","C_jambangan","C_jarakensis","C_jarujini","C_jelawangensis","C_jellesmae","C_kaba_genebank","C_khasiensis","C_kimberleyensis","C_kingsadai","C_klugei","C_langkawiensis","C_leegrismeri","C_lekaguli","C_lenya","C_lomyenensis","C_loriae","C_louisiadensis","C_macrotuberculatus","C_majulah","C_malayanus","C_marmoratus","C_martini","C_mcdonaldi","C_medioclivus","C_metropolis","C_mimikanus","C_murua","C_nigriocularis","C_novaeguineae","C_oldhami","C_otai","C_pageli","C_pantiensis","C_papuensis","C_paradoxus","C_payacola","C_payarhtanensis","C_peguensis","C_petani","C_philippinicus","C_phongnhakebangensis","C_phuquocensis","C_pronarus","C_psarops_MZB_9687_Sumatra_Lampung_Ngarip","C_pseudoquadrivirgatus","C_pubisulcus","C_pulchellus","C_quadrivirgatus","C_quadrivirgatus_18659_Aceh","C_quadrivirgatus_18726_Kapahiang","C_redimiculus","C_rishivalleyensis","C_robustus","C_roesleri","C_rosichonariefi","C_rubidus","C_rufford","C_russelli","C_sadleiri","C_salomonensis","C_semenanjungensis","C_semicinctus_14966_Sumatera_Barat_Pasaman","C_seribuatensis","C_sermowaiensis","C_serratus","C_sharkari","C_slowinskii","C_sommerladi","C_soni","C_sp_14124_Sumatra_Lampung_Ngarip","C_sp_15836_Java_Jawa_Barat","C_srilekhae","C_sworderi","C_takouensis","C_tautbatorum","C_tebuensis","C_teyniei","C_thochuensis","C_tigroides","C_timur","C_tiomanensis","C_triedrus","C_trilatofasciatus","C_tripartitus","C_tuberculatus","C_vilaphongi","C_wayakonei","C_yangbayensis","C_yoshii","C_ziegleri","C_zugi","Cyrtopodion_himalayanum","Cyrtopodion_lawderanum","Cyrtopodion_tibetanus","Geckoella_albofasciatus","Geckoella_collegalensis","Geckoella_deccanensis","Geckoella_jeyporensis","Geckoella_nebulosus","Geckoella_triedrus")
pruned.tree<-drop.tip(tree,tree$tip.label[-match(species, tree$tip.label)])
#output pruned tree
write.tree(pruned.tree, file="expanded_pruned.tre")
#########################################

trfn = "expanded_pruned.tre"
geogfn = "ranges_expanded13areas.txt"
max_range_size = 3
...
#Phylo Areas
#these are my examples from my gecko analysis (not sure what the real ones are for the tutorial data)
#File 1 A-J
#species	Tibet	India	Indochina	Phillippine	Papua	Wallacea Pen_Malaysia	Sumatra	Java	Borneo

moref(geogfn)
tipranges = getranges_from_LagrangePHYLIP(lgdata_fn=geogfn)

#check number of states, if over 100, the analysis will be somewhat slow
numstates_from_numareas(numareas=4, maxareas=1, include_null_range=F)
...
#####This is the first analysis DEC
# For a slow analysis, run once, then set runslow=FALSE to just 
# load the saved result.
runslow = TRUE
resfn = "Cyrt_DEC.Rdata"
if (runslow)
{
...
resfn = "Cyrt_Dec+J_M0_unconstrained_v1.Rdata.Rdata"
runslow = TRUE
if (runslow)
{
...
# PDF plots
#######################################################
pdffn = "Cyrt_DEC_vs_DEC+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - DEC
#######################################################
analysis_titletxt ="BioGeoBEARS DEC on Cyrt M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DEC+J on Cyrt M0_unconstrained"
...
runslow = TRUE
resfn = "Cyrt_DIVALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Cyrt_DIVALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = "tutorial_DIVALIKE_vs_DIVALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on Cyrt M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on Cyrt M0_unconstrained"
...
runslow = TRUE
resfn = "Cyrt_BAYAREALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "Cyrt_BAYAREALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "Cyrt_BAYAREALIKE_vs_BAYAREALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Cyrt M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Cyrt M0_unconstrained"
